...
class PF_NODE(Node):
    def __init__(self):
        ...
        self.landmarks_sub = self.create_subscription(LandmarkArray, '/landmarks', self.update_step, 10)
        #sensor param
        ...
        std_range = 0.225  # [m]
        std_bearing = np.deg2rad(3.2)  # [rad]
        ...
        #self.control = np.array([0.000000001, 0.000000001])  # Default control: [linear_velocity, angular_velocity]
        
        self.pf.mu = np.array([0.0, 0.01, 0.01])  # x, y, theta
        self.pf.Sigma = np.diag([0.05, 0.05, 0.05])
        self.pf.initialize_particles()
        
        #self.sim_pos=None #self.pf.mu.copy()
    ...
    def update_step(self, msg2):
        ...
        for landmark in msg2.landmarks:
            #zeta = np.array([landmark.range, landmark.bearing])
            # Get landmark coordinates using the ID from the message
            landmark_coords = self.landmarks.get(landmark.id)
            self.get_logger().info(f"Processing landmark ID: {landmark.id}")
            
            landmark_x, landmark_y = landmark_coords
            self.get_logger().info(f"Landmark {landmark.id} coordinates: (x={landmark_x}, y={landmark_y})")
